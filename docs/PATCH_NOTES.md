...
- 24시간 합성 모니터링 스크립트 추가:
  - `scripts/synthetic_monitor.py`
  - Backend health, AI health, 인증 API 핵심 플로우를 주기적으로 점검
  - 성공률/연속 실패 기준으로 PASS/FAIL 판정 및 JSON 리포트 생성
- 단기 부하 테스트 스크립트 추가:
  - `scripts/load_test_short.py`
  - 동시 사용자(VU) 기반으로 핵심 API 응답시간/오류율 측정
  - 임계값(`maxErrorRate`, `maxP95ms`) 기준 PASS/FAIL 판정
- 월간 DR 리허설 실행 스크립트 추가:
  - `scripts/dr_rehearsal_run.ps1`
  - 합성 모니터링 + 부하 테스트 + 수동 확인 플래그(복구/무결성) 통합 점검
  - 요약 리포트(`overallPass`) 생성

## 문서 연동

- `docs/OPERATIONS_CHECKLIST.md`
  - 자동 검증 명령 섹션(실행 예시) 추가
  - DR 점검 항목에 리허설 리포트 보관 기준 추가
- `docs/OPERATIONS_MANUAL.md`
  - 자동 검증 도구 섹션 추가(스크립트/리포트 경로 안내)

---

# v1.3.5 - 운영 체크리스트 확장 (전체 기능 검증 매트릭스)
...
- `docs/OPERATIONS_CHECKLIST.md`를 확장형 체크리스트로 재구성
  - 일일 빠른 점검(10~15분)
  - 배포 전/후 GO-NO-GO 점검
  - 전체 기능 검증 매트릭스(Auth/2FA/분석/백테스트/포트폴리오/워치리스트/모의투자/챗봇/스크리너/실시간)
  - 프론트 UX/데이터 무결성/성능/보안/관측성/복구/DR 항목 추가
- `docs/OPERATIONS_MANUAL.md`에 확장 체크리스트 사용 안내 추가
